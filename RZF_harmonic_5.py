...
# Configuration
M = 5           # Number of known zeros to use
gamma_min = 10.0
gamma_max = 40.0
num_points = 1000
...
# Initial parameters
initial_alpha = 1.0
initial_beta_min = 0.1
initial_beta_max = 2.0
initial_N = 500

class App:
    def __init__(self, master):
        self.master = master
        master.title("Gaussian-Modulated Harmonic Approximation")
        ...
        # Draw initial plot
        self.plot_function(initial_alpha, initial_beta_min, initial_beta_max, initial_N)

    def plot_function(self, alpha, beta_min, beta_max, N):
        # Compute the function with given parameters
        ...
        for i, g in enumerate(gamma_values):
            exp_part = np.exp(-alpha * np.pi * g**2)
            for j, b in enumerate(betas):
                Phi[i,j] = exp_part * np.cos(2*np.pi*b*g)
        ...
        def F(g):
            exp_part = np.exp(-alpha * np.pi * g**2)
            return np.sum(c * exp_part * np.cos(2*np.pi*betas*g))
        ...
        self.ax.set_title('Adjust Parameters with Sliders')
    ...
    def update_plot(self, event=None):
        ...
        self.plot_function(alpha, beta_min, beta_max, N)
