...
def refine_with_newton(harmonics, steps=5):
    """Refine harmonics using the Newton-Raphson method."""
    refined_harmonics = []
    for freq in harmonics:
        t = freq  # Initial guess for harmonic frequency
        for _ in range(steps):
            s = 0.5 + 1j * t  # Point on the critical line
            z_val = zeta(s)   # Riemann zeta function value
            z_deriv = diff(zeta, s)  # Riemann zeta function derivative
            
            # Newton-Raphson update: t = t - f(t)/f'(t)
            t -= float(z_val.imag / z_deriv.imag)
        refined_harmonics.append(t)
    return np.array(refined_harmonics)
...
if __name__ == "__main__":
    ...
    for iteration in range(iterations):
        ...
        # Refine harmonics using Newton-Raphson
        refined_peaks = [pf for pf, _ in matched_zeros]
        harmonics = refine_with_newton(refined_peaks)
        harmonics = np.unique(harmonics)[:initial_harmonics + top_peaks_limit]

    # Step 7: Visualization - FFT Peaks and True Zeros
    ...
    # Step 8: Visualization - Error Reduction Over Iterations
