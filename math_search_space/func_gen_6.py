...
def is_just_unary_function(expr):
    ...
    # expr.func gives the top-level function. If it's sin, cos, or log, we return True.
    if expr.func in [sin, cos, log]:
        # It's just a unary operation at the top-level
        return True
    return False
...
def generate_non_constant_and_non_linear_expression(max_ops=3):
    ...
    while True:
        ...
        # Filter out constant and linear functions
        if is_constant_function(expr):
            continue
        if is_linear_function(expr):
            continue
        # Filter out expressions that are just a unary function at the top-level
...
def plot_expression(expr_str):
    ...
    plt.title('Randomly Generated Non-Constant, Non-Linear Expression (No Top-Level Unary Only)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.grid(True)
    plt.legend()
...
if __name__ == "__main__":
    ...
    print("Generated non-constant, non-linear equation (no trivial unary):", expr_str)
